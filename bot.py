...
# ---------------- الحالات (FSM) ----------------
class GenerateVideo(StatesGroup):
...
async def my_account(callback: CallbackQuery):
    ...
    if free:
        lines.append("🆓 أنت من ضمن المستخدمين المميزين، بتستخدم البوت مجاناً بدون خصم من رصيدك.")
    text = "\n".join(lines)
...
# ---------------- توليد الفيديو ----------------
def user_can_generate(telegram_id: int) -> bool:
    return db.is_free_user(telegram_id) or db.get_credits(telegram_id) > 0


@dp.callback_query(F.data == "gen_video")
async def gen_video_start(callback: CallbackQuery, state: FSMContext):
    if not user_can_generate(callback.from_user.id):
        await callback.message.edit_text(
        ...
        return
    await state.set_state(GenerateVideo.waiting_prompt)
    await callback.message.edit_text(
        "🎬 وصف الفيديو\n\n"
...
@dp.message(GenerateVideo.waiting_prompt, F.text)
async def gen_video_prompt(message: Message, state: FSMContext):
    await state.update_data(prompt=message.text)
    await state.set_state(GenerateVideo.waiting_duration)
    await message.answer("⏱ اختار مدة الفيديو:", reply_markup=duration_kb())


@dp.callback_query(GenerateVideo.waiting_duration, F.data.startswith("dur_"))
async def gen_video_duration(callback: CallbackQuery, state: FSMContext):
    duration = int(callback.data.split("_")[1])
    await state.update_data(duration=duration)
    await state.set_state(GenerateVideo.waiting_confirm)

    data = await state.get_data()
    prompt = data.get("prompt", "")
    free = db.is_free_user(callback.from_user.id)
    credits = db.get_credits(callback.from_user.id)
    cost_line = "🆓 مجاني (من ضمن المستخدمين المميزين)" if free else "1 فيديو من رصيدك"
...
@dp.callback_query(GenerateVideo.waiting_confirm, F.data == "gen_cancel")
async def gen_video_cancel(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    welcome = db.get_setting("welcome_text", "أهلاً بيك 👋")
    await callback.message.edit_text(welcome, reply_markup=main_menu_kb())


@dp.callback_query(GenerateVideo.waiting_confirm, F.data == "gen_confirm")
async def gen_video_confirm(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    prompt = data.get("prompt", "")
    ...
    free = db.is_free_user(telegram_id)

    if not free:
        # نتأكد تاني وقت التنفيذ إن الرصيد لسه موجود (يمنع التلاعب لو حصل تأخير)
    ...
    gen_id = db.add_generation(telegram_id, prompt, duration)
    try:
        video_url = await asyncio.to_thread(generate_video, prompt, duration)
