...
def init_db():
    with get_conn() as conn:
        ...
        # ترقية آمنة لو قاعدة بيانات قديمة كانت موجودة من غير عمود kind
        try:
            c.execute("ALTER TABLE generations ADD COLUMN kind TEXT DEFAULT 'video'")
...
# ---------------- توليد الفيديوهات ----------------
def add_generation(telegram_id, prompt, duration=None, kind="video"):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO generations (telegram_id, prompt, duration, kind, created_at) VALUES (?,?,?,?,?)",
            (telegram_id, prompt, duration, kind, int(time.time())),
        )
        return c.lastrowid
