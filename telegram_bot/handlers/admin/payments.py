...
async def _fulfill_payment(session, user, payment, bot=None) -> None:
    ...
    if purpose == "code_generation":
        ...
        user.project_free_credits += 1
        await session.commit()
        if bot:
            try:
                await bot.send_message(
                    user.telegram_id,
                    "✅ تم تأكيد دفعتك! يمكنك الآن الدخول إلى 🧱 مولّد المشاريع AI لاستخدامه.",
                )
            except Exception:
                pass
            await announce_to_channel(bot, session, "🧱 تم تأكيد دفعة مولّد مشاريع يدويًا.")
