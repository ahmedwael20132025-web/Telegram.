...
async def manage_file(callback: CallbackQuery, session) -> None:
    ...
    text = (
        ...
        f"👑 حصري لـ VIP مجانًا: {'نعم' if item.vip_only_free else 'لا'}"
    )
    rows = [
        [InlineKeyboardButton(text="🔁 تفعيل/تعطيل", callback_data=f"file_toggle:{file_id}")],
        [InlineKeyboardButton(text="👑 تبديل حصرية VIP المجانية", callback_data=f"file_vip_toggle:{file_id}")],
...
@router.callback_query(F.data.startswith("file_vip_toggle:"))
async def toggle_file_vip_only(callback: CallbackQuery, session) -> None:
    file_id = int(callback.data.split(":")[1])
    repo = CodeFileRepository(session)
    item = await repo.get(file_id)
    if item:
        await repo.update_fields(file_id, vip_only_free=not item.vip_only_free)
