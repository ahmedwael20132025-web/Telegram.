...
async def start_handler(message: Message, session, user: User) -> None:
    unsubscribed = await get_unsubscribed_channels(message.bot, session, user.telegram_id)
    if unsubscribed:
...
async def check_subscription_callback(callback: CallbackQuery, session, user: User) -> None:
    unsubscribed = await get_unsubscribed_channels(callback.bot, session, user.telegram_id)
    if unsubscribed:
