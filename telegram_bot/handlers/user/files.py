...
async def list_files_in_category(callback: CallbackQuery, session, user: User) -> None:
    ...
    rows = [
        [InlineKeyboardButton(
            text=(f"👑 {f.title} (VIP مجانًا)" if f.vip_only_free else f"📦 {f.title} — {format_money(f.price)}"),
            callback_data=f"fileview:{f.id}",
        )]
        for f in files
    ]
...
async def view_file(callback: CallbackQuery, session, user: User) -> None:
    ...
    if item.vip_only_free:
        if is_vip:
            await repo.increment_downloads(file_id)
            await purchase_repo.create(user.id, "file", file_id, 0.0)
            await callback.message.answer_document(
                item.file_id, caption=f"👑 {item.title} (حصري لأعضاء VIP - مجانًا)"
            )
            await callback.answer()
            return
        else:
            ...
            await callback.message.answer(
                f"🔒 ملف \"{item.title}\" حصري لأعضاء VIP فقط. اشترك للحصول عليه مجانًا.",
...
async def choose_file_payment(callback: CallbackQuery, state: FSMContext, session, user: User) -> None:
    ...
    if method.code == "stars":
        ...
        await send_stars_invoice(
            ...
            title="شراء ملف برمجي", description="شراء ملف من متجر الملفات البرمجية",
            payload=f"file:{file_id}:{user.id}", amount_stars=int(price),
        )
        await callback.answer()
        return
